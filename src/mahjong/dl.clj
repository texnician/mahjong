...
(defn parse-triples [ast]
  ...
  (let [suit (first (.getChildren ast))
        suit-sym (-> suit .getText clojure.string/upper-case symbol)
        tile-enums (map #(-> % .getText java.lang.Integer/parseInt) (.getChildren suit))
        tile-num (.getChildCount suit)]
    (cond (pong? tile-enums) [:pong suit-sym (first tile-enums)]
          (chow? tile-enums) [:chow suit-sym [(first tile-enums) (second tile-enums) (nth tile-enums 2)]]
          (kong? tile-enums) [:pub-kong suit-sym (first tile-enums)]
          :else (assert false))))

(defn parse-free-tiles [ast]
  "Parse free tiles. ([ast]) 3456b -> [:free-tiles 'B [3 4 5 6]]"
  (let [suit-sym (-> ast .getText clojure.string/upper-case symbol)
        tile-enums (map #(-> % .getText java.lang.Integer/parseInt) (.getChildren ast))]
    [:free-tiles suit-sym tile-enums]))
...
(defn parse-kong [ast]
  ...
  (let [suit (first (.getChildren ast)) 
        ...
        tile-enums (map #(-> % .getText java.lang.Integer/parseInt) (.getChildren suit))]
    (if (kong? tile-enums)
      [:kong suit-sym (first tile-enums)]
                            ...
                            (cond (= comb-type :chow)
                                  ...
                                  (let [free-tiles (make-free-tiles)]
                                    (let [tile-seq (partition 2 (apply concat (map (fn [x]
                                                                                     (let [[_ suit enums] x]
                                                                                       (interleave enums (repeat suit))))
                                                                                   comb-list)))]
                                      (loop [tiles tile-seq free-tiles (make-free-tiles)]
                                        (if (empty? tiles)
                                          free-tiles
                                          (let [[enum suit] (first tiles)]
                                            (recur (next tiles) (add-tile free-tiles enum suit))))))))]
                           ))
